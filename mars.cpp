...
uint32_t mars::rotl32(uint32_t value, unsigned int shift)
{
    shift &= 31;//если 32 то сдвиг на 0
    return shift == 0 ? value : (value << shift) | (value >> (32 - shift));
}
uint32_t mars::rotr32(uint32_t value, unsigned int shift)
{
    shift &= 31;
    return shift == 0 ? value : (value >> shift) | (value << (32 - shift));
}
uint32_t mars::load32(const byte_array& data, size_t offset)
{//загружает 32 битное из массива по смещению офсет в порядке литэнджен берет байт по индексу и освобождает место
    return static_cast<uint32_t>(data[offset]) |
        (static_cast<uint32_t>(data[offset + 1]) << 8) |
        (static_cast<uint32_t>(data[offset + 2]) << 16) |
        (static_cast<uint32_t>(data[offset + 3]) << 24);
}
void mars::store32(byte_array& data, size_t offset, uint32_t value)
{
...
mars::mars(const byte_array& key)
    : key_size(key.size())
{//проверка ключа
    if (key_size < 16 || key_size > 56 || key_size % 4 != 0)
        throw std::invalid_argument("invalid MARS key size");
    generate_keys(key);
}
void mars::generate_keys(const byte_array& key)
{//40 раундовых ключей
    uint32_t t[15] = {};//в алг испол 15 слов для расширения ключа
    size_t words = key.size() / 4;//вычисляет кол-во 32-битных слов в ключе
    for (size_t i = 0; i < words; ++i)
        t[i] = load32(key, i * 4);//заполняет первые вордс элементов массива t словами из ключа 
    t[words] = static_cast<uint32_t>(words);//записывет в эл знач вордс (кол-во слов) доп инф для смешивания
    for (size_t j = 0; j < 4; ++j)//4 прохода расширения ключа каждый проход ген 10 раунд ключей
    {
        for (size_t i = 0; i < 15; ++i)
            t[i] ^= rotl32(t[(i + 8) % 15] ^ t[(i + 13) % 15], 3) ^ static_cast<uint32_t>(4 * i + j);//перемешивает знач t
        for (size_t k = 0; k < 4; ++k)//сильное перешивание с испол s-box еще один вложенный цикл 4 повт для всех 15 жл 
            for (size_t i = 0; i < 15; ++i)
                t[i] = rotl32(t[i] + s_box[t[(i + 14) % 15] & 511], 9);//берется эл из s-box по индексу с бит маской до 512 эл прибавляется к t[i] знач из s-box по этому индексу (слож по модулю 2^32) затем рез цикл сдвигается влево на 9 бит
        for (size_t i = 0; i < 10; ++i)//после внутренних преоб для тек прохода j из массива t выбирается 10 слов по опр индекса и сохр в рауд кейс начиная с поз 10*j, получаем 10 ключ для каждого прохода
            round_keys[10 * j + i] = t[(4 * i) % 15];
    }
    for (size_t i = 5; i < 37; i += 2)
    {//доп корректирова ключей, для нечетных индексов от 5 до 35 с шагом 2
        uint32_t w = round_keys[i] | 3;//берет раунд ключ и устанавливает два младших бита в 1 побит ИЛИ с 3 ключ нечетный и не кратен 4
        uint32_t m = (~w ^ (w << 1)) & (~w ^ (w >> 1)) & 0x7ffffffe;//вычисляет маску побит НЕ с w, и сдвиги, находятся биты, которые отличаются от соседних,обнуляет старший и мл бит, оставляя биты с 1 по 30
        m &= m >> 1;
        m &= m >> 2;
        m &= m >> 4;//находит маску где устан биты, которые после неск сдвигов остаются единицами (т.е длинные послед единиц)
        m |= m << 1;
        m |= m << 2;
        m |= m << 4;//распр установл влево заполняя промежутки (обр расшир маски(
        m &= 0x7ffffffc;//обнуляет мл 2 бит а и старший 0,1 и 31 обнулены
        w ^= rotl32(s_box[265 + (round_keys[i] & 3)], round_keys[i - 1]) & m;//вычисл знач из s-box по индексу 256+(round_keys[i] & 3) берет один из четырех эл начиная с 265 применяет к нему цикл сдвиг на round_keys[i-1] бит (взятие младших 5, затем иасскируем m и хор с w
        round_keys[i] = w;
    }
}
byte_array mars::encrypt_block(const byte_array& block) const
{
    if (block.size() != 16)
        throw std::invalid_argument("invalid block size");
    uint32_t a = load32(block, 0);//загр 32-битные слова в порядке литэнджен
    uint32_t b = load32(block, 4);
    uint32_t c = load32(block, 8);
    uint32_t d = load32(block, 12);
    a += round_keys[0];//добавляет первые четыре раунд ключа к соотв словам начальное смешивание ключа с данными
    b += round_keys[1];
    c += round_keys[2];
    d += round_keys[3];
    for (size_t i = 0; i < 8; ++i)
    {//по 8 раундам
        b = (b ^ s_box[a & 255]) + s_box[((a >> 8) & 255) + 256];//на а извлек младший (a & 255) и второй байт ((a >> 8) & 255)  + 256 вторая половина s-box.  b хорится с первым затем к нему второе 
        c += s_box[(a >> 16) & 255];//к с прибавляется знач по индексу третьего байта а
        a = rotr32(a, 24);//цикл сдвиг а вправо на 24 бита (влево на 8 бит) меняет байтовый порядок
        d ^= s_box[(a & 255) + 256];//берем мл бит нового а к нему прибавляем вторую половину s-box и полученное хор с d
        a += (i % 4 == 0) ? d : 0;
        a += (i % 4 == 1) ? b : 0;//в зав от номера раунда i (по модулю 4) к а приб d или b
        uint32_t t = a;
        a = b;
        b = c;
        c = d;
        d = t;
    }//перестановка цикл сдвиг регистров
    for (size_t i = 0; i < 16; ++i)
    {
        uint32_t t = rotl32(a, 13);//t рез циклического сдвига влево на 13 бит
        uint32_t r = rotl32(t * round_keys[2 * i + 5], 10);
        uint32_t m = a + round_keys[2 * i + 4];
        uint32_t l = rotl32(s_box[m & 511] ^ rotr32(r, 5) ^ r, r);//берем младшие 9 бит
        c += rotl32(m, rotr32(r, 5));
        if (i < 8)//в зав от номера
        {
            b += l;
            d ^= r;
        }
    ...
    for (size_t i = 0; i < 8; ++i)//8 обр раундов
    {
        a -= (i % 4 == 2) ? d : 0;
        a -= (i % 4 == 3) ? b : 0;
        b ^= s_box[(a & 255) + 256];
        c -= s_box[(a >> 24) & 255];//из с вычитается знач бокса по индексу старшего байта а
        uint32_t t = rotl32(a, 24);
        d = (d - s_box[((a >> 16) & 255) + 256]) ^ s_box[t & 255];//из d вычит знач бокса по индексу третьего байта а + 256 затем хор с боксом по индексу младшего байта t
        a = b;
        b = c;
        c = d;
        d = t;
    }
    a -= round_keys[36];//вычит послед четырех раунд ключеей из соотв слов
    b -= round_keys[37];
    c -= round_keys[38];
    d -= round_keys[39];
...
byte_array mars::decrypt_block(const byte_array& block) const
{
    if (block.size() != 16)
        throw std::invalid_argument("invalid block size");
    uint32_t d = load32(block, 0);//загружает слова но порядок обратный
    uint32_t c = load32(block, 4);
    uint32_t b = load32(block, 8);
    uint32_t a = load32(block, 12);
    d += round_keys[36];//добавляет последние четыре раунд ключа к словам
    c += round_keys[37];
    b += round_keys[38];
    a += round_keys[39];
    for (size_t i = 0; i < 8; ++i)
    {//8 обр раундов первая фаза дешифр совпадает с первой фазой шифр испол одни и те же раунды но с разным порядком ключей
        b = (b ^ s_box[a & 255]) + s_box[((a >> 8) & 255) + 256];
        c += s_box[(a >> 16) & 255];
        a = rotr32(a, 24);
        d ^= s_box[(a & 255) + 256];
        a += (i % 4 == 0) ? d : 0;
    ...
    for (size_t i = 0; i < 16; ++i)
    {//16 рауднов слож заменены на вычитаня порядок ключей обратный
        uint32_t t = rotr32(a, 13);//сдвиг а вправо не налево
        uint32_t r = rotl32(a * round_keys[35 - 2 * i], 10);//ключм в обр порядке 35,33,31 рез сдвиг на 10 бит
        uint32_t m = t + round_keys[34 - 2 * i];
        uint32_t l = rotl32(s_box[m & 511] ^ rotr32(r, 5) ^ r, r);
        c -= rotl32(m, rotr32(r, 5));//инверсия опер шифрования
        if (i < 8)
        {
            b -= l;
            d ^= r;
    ...
    for (size_t i = 0; i < 8; ++i)
    {//вторая фаза обратного смешивания аналогична третьей фазе шифрования
        a -= (i % 4 == 2) ? d : 0;
        a -= (i % 4 == 3) ? b : 0;
        b ^= s_box[(a & 255) + 256];
        c -= s_box[(a >> 24) & 255];
        uint32_t t = rotl32(a, 24);
        d = (d - s_box[((a >> 16) & 255) + 256]) ^ s_box[t & 255];
        a = b;
        b = c;
        c = d;
        d = t;
    }
    d -= round_keys[0];//вычитание первых четырех раунд клюей из соотв слов
    c -= round_keys[1];
    b -= round_keys[2];
    a -= round_keys[3];
    byte_array result(16);
    store32(result, 0, d);//сохраняет слова в обр порядке
    store32(result, 4, c);
    store32(result, 8, b);
    store32(result, 12, a);
    return result;
}
size_t mars::get_block_size() const
{
    return 16;
}
size_t mars::get_key_size() const
{
    return key_size;
}
size_t mars::get_round_count() const
{
    return 32;
}
